...
Package: roundR
Type: Package
Title: This package provides a function that rounds to the nearest integer
Version: 0.1.0
Author: Gerko Vink and Hanne Oberman
...
#' examplecode 2
```
which for our function `rounder()` would result in the following `rounder.R` code file
...
Let's publish our `R`-package to `GitHub`. 

### Registering a `DOI`

### Add citation
The next step before we would put our package *out there* is to notify how users can refer to our package. Run the following code:
```{r eval = FALSE}
